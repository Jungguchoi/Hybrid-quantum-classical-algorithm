...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x_splitted_2 = x[0][32:].view(1,1,108)
        
        ## 1) classical conv layer
        x_classic = torch.tanh(self.conv1(x_splitted_2))
        x_classic = torch.tanh(self.conv2(x_classic))
        x_classic = torch.tanh(self.conv3(x_classic))
        ...
        concatted_x = torch.cat([x_1, x_2, x_classic]).view(1, 36)
...
root_dir = "G://220911_fMRI_QC/0_data/1_results_221115_CN_EMCI/"
...
def save_fig(loss_list, mode, cv_no):
    ...
    plt.savefig('G://220911_fMRI_QC/2_result/0_figure/1_COND2_VQC2_front/Test_Figure_loss_'+str(mode)+'_'+str(cv_no)+'CV_'+str(ansatz)+'_.png')
...
for no, file in enumerate(filelist[20:]):
    ...
    for no_, (train_index, test_index) in enumerate(skf.split(signal_data, signal_label)):
        ...
        inter_list_train_df.to_csv('G://220911_fMRI_QC/2_result/1_loss/1_COND2_VQC2_front/Test_per_epoch(ROI_'+str(ROI_info)+')_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False, header=None)
    
        save_dir = 'G://220911_fMRI_QC/2_result/2_trained_model/1_COND2_VQC2_front/Trained_model_ROI_'+str(ROI_info)+'_'+str(no_)+'CV_'+str(ansatz)+'.pth'
        torch.save(trained_hybrid_model.state_dict(), save_dir)
        
        train_loss_df = pd.DataFrame(loss_list_train)
        train_loss_df.to_csv('G://220911_fMRI_QC/2_result/1_loss/1_COND2_VQC2_front/Train_loss(ROI_'+str(ROI_info)+')_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False, header=None)
        ...
        exp_results_df.to_csv('G://220911_fMRI_QC/2_result/3_performance/1_COND2_VQC2_front/performance_ROI_'+str(ROI_info)+'_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False)
