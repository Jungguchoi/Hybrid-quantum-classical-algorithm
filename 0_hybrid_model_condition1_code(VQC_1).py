...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x_splitted_1 = x[0][16:].view(1,1,124)
        
        ## 1) classical conv layer
        x_classic = torch.tanh(self.conv1(x_splitted_1))
        ...
        concatted_x = torch.cat([x_1, x_classic]).view(1, 98)
...
def save_fig(loss_list, mode, cv_no):
    ...
    plt.savefig('F:/Student/CJG/220916_QC/3_IBM_QLAB/2_result/0_hybrid/0_figure/1_COND1_VQC1_front/Test_Figure_loss_'+str(mode)+'_'+str(cv_no)+'CV_'+str(ansatz)+'_.png')
...
for no, file in enumerate(filelist[68+3+10+27:]):
    ...
    for no_, (train_index, test_index) in enumerate(skf.split(signal_data, signal_label)):
        ...
        inter_list_train_df.to_csv('F:/Student/CJG/220916_QC/3_IBM_QLAB/2_result/0_hybrid/1_loss/1_COND1_VQC1_front/Test_per_epoch(ROI_'+str(ROI_info)+')_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False, header=None)
    
        save_dir = 'F://Student/CJG/220916_QC/3_IBM_QLAB/2_result/0_hybrid/1_trained_model/1_COND1_VQC1_front/Trained_model_ROI_'+str(ROI_info)+'_'+str(no_)+'CV_'+str(ansatz)+'.pth'
        torch.save(trained_hybrid_model.state_dict(), save_dir)
        
        train_loss_df = pd.DataFrame(loss_list_train)
        train_loss_df.to_csv('F:/Student/CJG/220916_QC/3_IBM_QLAB/2_result/0_hybrid/1_loss/1_COND1_VQC1_front/Train_loss(ROI_'+str(ROI_info)+')_'+str(no_)+'CV_'+str(ansatz)+'.csv', index=False, header=None)
